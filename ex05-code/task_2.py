...
def preview_dognames(list, n):
    ...
    """
    return [x['HUNDENAME'] for x in list[:n]]
...
def dognames_count(list):
    ...
    name_list = [name['HUNDENAME'] for name in list]
    return {key: name_list.count(key) for key in name_list}
...
def top_n_dognames(list, n):
    ...
    top_list = dognames_count(list)
    sorted_top_list = sorted(top_list.items(), key=operator.itemgetter(1), reverse=True)
    return sorted_top_list[:n]
...
def is_valid_row(row, year=None, sex=None):
    ...
    if sex is None and year is None:
        return True
    elif row['GEBURTSJAHR_HUND'] == str(year) or row['GESCHLECHT_HUND'] == sex:
        return True
    elif row['GEBURTSJAHR_HUND'] == str(year) and row['GESCHLECHT_HUND'] == sex:
        return True
    else:
        return False
...
def filter_dognames(list, year=None, sex=None):
    ...
    :return:
    """
    return [item for item in list if is_valid_row(item, year, sex)]
...
if __name__ == '__main__':
    ...
    print(filter_dognames(dognames, 2004))

    # test is_valid_row function
    print(is_valid_row(row=dognames[1], year=2013, sex='w'))
